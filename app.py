...
st.divider()
st.subheader("Taux de réussite historique")

win_ratio_df = pd.read_csv("win_ratio_par_entite.csv")
win_ratio_df = win_ratio_df[win_ratio_df["nb_missions"] >= 30]
win_ratio_df["entite"] = win_ratio_df["entityTrig_code"].map(
    {v: k for k, v in entityTrig_map.items()}
)

col1, col2 = st.columns(2)
with col1:
    st.metric("Win ratio global", "25.0 %")
with col2:
    st.metric("Missions historiques analysées", int(win_ratio_df["nb_missions"].sum()))

st.bar_chart(win_ratio_df.set_index("entite")["win_ratio"])
st.dataframe(win_ratio_df[["entite", "nb_missions", "win_ratio"]])
